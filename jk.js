(function () {
  ...
  // smooth, gentle ease for a graceful anime-style blink
  ...
  const CLOSE_DUR = 650;   // ms - slow graceful close
  const CLOSED_DUR = 260;  // ms - brief pause while shut
  const OPEN_DUR = 700;    // ms - slow graceful open
  ...
  function updateBlink(dt) {
    ...
    if (blinkState === 'open') {
      ...
      if (!patternSwitchedThisBlink) {
        if (Math.random() < 0.5) mode = (mode + 1) % modes.length;
  ...
  // ---------------- eye shape: matches reference photo ----------------
  // Sharp pointed corners; inner corner (toward center of face) sits higher
  // than the outer corner, giving that narrow, fierce, upward-tilted look.
  function eyePath(ew, eh, openAmt) {
    ...
    ctx.beginPath();
    ctx.moveTo(-ew * 0.5, outerY); // outer (far) corner - sharp point
    ctx.quadraticCurveTo(-ew * 0.18, -upPeak, ew * 0.12, -upPeak * 0.86);
    ctx.quadraticCurveTo(ew * 0.38, -upPeak * 0.48, ew * 0.5, innerY); // inner corner - sharp, elevated
  ...
  function drawEye(cx, cy, ew, eh, mirror, patternName, rotSign, t) {
    ...
    // --- flat white sclera, exactly filling the pointed shape ---
    ctx.save();
    ctx.clip();
    ctx.fillStyle = '#f7f4ef';
    ctx.fillRect(-ew, -eh, ew * 2, eh * 2);

    // very subtle depth shadow tucked at the very top corner (kept minimal
    // so the white reads clean/flat like the reference, not painterly)
    ...
    // --- iris, centered inside the shape ---
    if (openAmt > 0.03) {
    ...
    // --- crisp black outline (reads as clean hand-drawn anime line) ---
  ...
  // simple scattered dots, matching the reference art exactly
